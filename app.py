...
# 2. Día Operativo (ARS + Conversión USD a BNA)
...
# Conversión del Dron (Punto solicitado: Dólar BNA Vendedor)
valor_usd = db_drones_usd[dron_sel]
valor_ars_bna = valor_usd * tc_bna
amortizacion_ars = valor_ars_bna * 0.001 # 0.1% de amortización diaria
...
with c3:
    st.metric(f"EQUIPO: {dron_sel}", f"u$s {valor_usd:,.2f}", delta=f"ARS {valor_ars_bna:,.2f}")
...
resumen = pd.DataFrame({
    ...
    "Costo Origen": [f"$ {costo_comb_dr:,.2f}", f"$ {costo_mant_dr:,.2f}", f"$ {viaticos_ars:,.2f}", f"$ {comb_gen_ars:,.2f}", f"$ {conectividad_ars:,.2f}", f"u$s {valor_usd:,.2f}"],
